...
Hace unos meses, escribí un blog post acerca de un análisis de [la brecha salarial de género en puestos académicos de la UNAM en el 2019](http://alejandroreyes.org/brecha-salarial-en-la-unam-en-el-2019/). Dudo que la Dra. Alvarez-Buylla, directora del *CONACYT*, haya leído mi blog post. Sin embargo, la Dra. Alvarez-Buylla escribió el día siguiente en twitter:
...
El gráfico demuestra el gran reto al que se enfrenta la "transformación virtuosa" de *CONACYT*. En general, **entre más alta la distinción del *SNI* menos mujeres hay**. Hay áreas en las que el sesgo es aún mayor, de las cuales describo tres áreas con patrones interesantes:
  ...
  + **En el área de biotecnología y ciencias de la tierra, el `r round(dat$percent_females[dat$area == "VI: BIOTECNOLOGÍA/AGROPECUARIAS" & dat$Nivel == "C"])` de los candidatos a doctor son mujeres, sin embargo este porcentage cae rápidamente conforme la distinción es mas prestigiosa: solo el `r round(dat$percent_females[dat$area == "VI: BIOTECNOLOGÍA/AGROPECUARIAS" & dat$Nivel == "3"])`% de los investigadores con *SNI* 3 son mujeres**. 
