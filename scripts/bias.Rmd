...
encuestas2018b <- encuestas2018b %>%
  dplyr::mutate( `Todos - AMLO` = rowSums(.[,!grepl("AMLO|Date|pollster", colnames(.))], na.rm=TRUE)) %>%
  dplyr::select( pollster, Date, AMLO, `Todos - AMLO` )

plotTrends( encuestas2018b, fechaLim="2017-11-01") + ylim(20, 70) + scale_x_date(date_breaks = "4 week", date_labels = "%b %d") + theme( axis.text.x=element_text(angle=25, hjust=1) )
