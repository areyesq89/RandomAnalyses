...
The National Autonomous University of Mexico (UNAM, for its accronym in spanish) is one of the biggest universities in Latin America. More than 10,000 scientist do their research in UNAM's laboratories. UNAM is funded from public spending and it is required by law to release all information about contracts and wages of each investigator. These data are available in the [UNAM's transparency website](http://www.transparencia.unam.mx/obligaciones/consulta/remuneracion-profesores). 

The current blog post describes a documented data analysis that addresses the question: How is UNAM doing in paying equally between genders?
...
I generated a catalog of names that are typically assigned to either males or females. Since Wikipedia has catalogs of male names and female names in several languages, I wrote a function to scraple these data. I ran these function for names in Spanish, English, Italian, Portughese, French and German.  
...
I noticed that many names that are common in Mexico were not in the Wikipedia pages, so I added manually all names that appeared more than 5 times in the wage list of UNAM's professors. I also discarded names that are given to both males and females. For some of these ambiguous cases, I assigned a gender based on what I though was more common in Mexico. An example of these exceptions is the name "Angel", which wikipedia lists it as a name for both females and males and  "Angel" is more common for males in Mexico. 
...
Since UNAM's wage list has full names for each researcher, I used my name catalog to assign a potential gender to each researcher. It is common for people to have more than one first name. Names like "Maria Jose" can be tricky, since "Maria" is a female name, "Jose" is a male name and "Maria Jose" is a female name. To resolve this cases, I assigned genders based on two strategies: (1) the gender of the first first name and (2) the consensus gender of the individual names. In general, these two strategies were highly concordant. 
...
Based on these strategies, I could assign a gender to `r sum(payData$gender %in% c("Male", "Female"))` people, which corresponds to `r 100*(sum(payData$gender %in% c("Male", "Female"))/nrow(payData))` of UNAM's academics. Of the people that I could assign a gender, `r sum(payData$gender %in% "Female")` were females.
...
Going right to the point, there is a significant difference between the wages earned by females and males ($p =$ `r pval`). On average, males earn `r meanDiff` MXN more than females. The plot below shows the distribution of wages for each gender. The data shows two peaks towards the right of the plot (better salaries) that are higher in males compared to females. These pattern is consistent with higher rank positions being dominated by males. 
...
The data has also information of the contract of each researcher. To see whether higher rank positions were dominated by males, I plotted the percentage of females in each contract as a function of the average salary in that contract. We can see that the split between males and females is close to 50% for most positions. Nevertheless, the percentage of females for the 5 better-payed positions is way less than 50%. 
...
payPerGenderSumm %>%
  ...
  ylim(0, 100) +
  labs(x="Average salary (1,000 MXN)", y="Percentage of females")
...
#  geom_errorbar(aes(ymin=conf.low/1000, ymax=conf.high/1000), width=.1) +
  ...
  geom_hline(yintercept=0, col="#88000040", alpha=0.8) +
  ylim(-2.8, 2.8) +
