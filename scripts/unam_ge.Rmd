...
When I tested for differences in wages between males and females with the same contracts, it turns out that **females earn on average `r round(100*coefFemale/mean(payData %>% dplyr::filter( gender %in% c("Male", "Female"), contrato %in% testable ) %>% dplyr::pull(pago_total)))`% more than males with the same contracts**. This difference is more evident in two contracts, *Investigador Titular B* and *Profesor Titular C*,  where females earn `r round(differentPays$aveDiff[differentPays$contrato == "I6593 INVESTIGADOR TITULAR B TIEMPO COMPLETO"], 1)`% and `r round(differentPays$aveDiff[differentPays$contrato == "D6696 PROFESOR TITULAR C TIEMPO COMPLETO"], 1)`% more than males, respectively. 

**In the most positions where the percentage of males is large, which tend to be senior positions, the few women in those contracts tend to earn more than man**. This is very intriguing and I don't have a clear explanation for it. One possible scenario is that man are promoted more often than women. In such scenario female researchers would remain with lower rank positions for more time and would accumulate more salary increases. 

The solution is obvious: **in order to reach gender equality, UNAM should promote more women to the higher rank academic titles**. 
