...
These data contain information for `r nrow(payData)` researchers, including full names, monthly salaries, academic ranks and academic departments.
...
The coefficient of interest in the model is $\beta_{1}^{female}$, which estimates the difference in salary that females receive compare to males after adjusting for salary differences between different contracts, which are estimated by the ${\beta}_{j}^{contract}$ coefficients. 
...
In these two positions, *Investigador Titular B* and *Profesor Titular C*, the average of females is `r round(differentPays$aveDiff[differentPays$contrato == "I6593 INVESTIGADOR TITULAR B TIEMPO COMPLETO"], 1)`% and `r round(differentPays$aveDiff[differentPays$contrato == "D6696 PROFESOR TITULAR C TIEMPO COMPLETO"], 1)`% higher than the average of males, respectively. 
...
For `r round(100*sum(numAboveZero$estimate > 0)/nrow(numAboveZero))`% of the contracts (`r sum(numAboveZero$estimate > 0)` out of `r nrow(numAboveZero)`), the average was higher for females than for males (i.e. the dots are above the horizontal dotted line). This explains why the $\beta_{1}^{female}$ coefficient is significant in the linear model. However, the 95% confidence intervals overlap with the zero line for `r sum(Vectorize(dplyr::between)(0, numAboveZero$conf.low, numAboveZero$conf.high))` of the contracts, which is why only the remaining `r sum(!Vectorize(dplyr::between)(0, numAboveZero$conf.low, numAboveZero$conf.high))` contracts, *Investigador Titular B* and *Profesor Titular C*, are significant in the contract-wise tests. 

# Conclusions

**My analysis shows that among UNAM's faculty members, males earns `r round(100*meanDiff/mean(payData$pago_total[payData$gender=="Male"]))`% more than female faculty on average. This wage gap difference is explained by the high rank academic titles being strongly dominated by males.** For example, only 15% of UNAM's highest academic titles, *Investigadores Emeritos*, are females. 

When I tested for differences in wages between males and females with the same contracts, it turns out that **females earn on average `r round(100*coefFemale/mean(payData %>% dplyr::filter( gender %in% c("Male", "Female"), contrato %in% testable ) %>% dplyr::pull(pago_total)))`% more than males with the same contracts**. This difference is more evident in two contracts, *Investigador Titular B* and *Profesor Titular C*,  where females earn `r round(differentPays$aveDiff[differentPays$contrato == "I6593 INVESTIGADOR TITULAR B TIEMPO COMPLETO"], 1)`% and `r round(differentPays$aveDiff[differentPays$contrato == "D6696 PROFESOR TITULAR C TIEMPO COMPLETO"], 1)`% more than males, respectively. However, these differences in wages are minimal.

**In the contracts where the percentage of man is large, which tend to be senior positions, the few women in those contracts tend to earn more than man**. This is very intriguing and I don't have a clear explanation for it. One possible scenario is that man are promoted more often than women. In such scenario female researchers would remain with lower rank positions for more time and thus would accumulate more salary increases. 

The biggest problem, however, is the lack of representation of female faculty in senior positions: **in order to reach gender equality, UNAM should promote more women to the highest rank academic titles**. 
